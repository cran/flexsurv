...
\Abstract{ \pkg{flexsurv} is an R package for fully-parametric
  ...
  the standard \pkg{survival} package.  Censoring or left-truncation
  are specified in \code{Surv} objects.  Estimates and confidence
...
S(\mu(\mathbf{z}) t | \bm{\alpha})$ then it is an \emph{accelerated
...
the survival time may be interval-censored on $(t_i, t^{max}_i)$.  If
there is right-censoring but no left-censoring then $t^{max}_i$ is
infinite, so that $S(t^{max}_i)=0$, or if there is left-censoring but
no right-censoring then $t_i=0$.
...
The individual survival times are independent, so that \pkg{flexsurv} does not
...
fs1
@ 
For the Weibull (and exponential and log-normal)
distributions, \code{flexsurvreg} simply uses \code{survreg} in
\pkg{survival} to find the maximum likelihood estimates. 
The same model can be fitted as
<<>>=
...
form --- as a series of left-truncated survival times.  For each
...
specified, as in standard R modelling functions, using \code{weights}
...
defines functions beginning with \code{h} giving the hazard, and \code{H}
...
\begin{Scode}
fs3 <- flexsurvreg(Surv(recyrs, censrec) ~ group, anc=list(sigma= ~ group), 
                   data=bc, dist="gengamma")
\end{Scode}
...
\hline
    ...
    Weibull     & \code{shape, {\color{red}{scale}}}     & \code{dweibull} & \code{"weibull"} \\
...
in standard R functions like \code{predict.lm}.   Time-dependent 
...
\code{plot.flexsurvreg}.  This should be an R function, with optional
...
exponential, Weibull, Gompertz, and hazard- and odds-based spline
...
transformed baseline parameters $\gamma$.  Arguments to \code{optim}
...
\item[\code{name}] Name of the distribution.  In the first example below, 
  we use a log-logistic distribution, and the name is \code{"llogis"}. 
  Then there is assumed to be at least either 
  
  \begin{itemize}
  \item  a function to compute the probability density, which would be called called \code{dllogis} here, or
  ...
  In some cases, R's scoping rules may not find the functions in the
...
\item[\code{pars}] Character vector naming the parameters of the
  ...
  arguments of the R distribution function or functions, in the same order.
...
\item[\code{inits}] A function which provides plausible initial values
  ...
  Weibull \code{shape} and \code{scale} respectively \footnote{though Weibull models in 
    \code{flexsurvreg} are ``initialised'' by fitting the model with \code{survreg}.}.
...
We may want to fit a Weibull model like \code{fs1}, but with the proportional hazards parameterisation $S(t) =
...
needs to be detached first so that \pkg{flexsurv}'s built-in \code{hweibull} is used, which returns \code{NaN} if the parameter values are zero, rather than failing as \pkg{eha}'s currently does.}
<<>>=
detach("package:eha")
...
and the parameter estimates of \code{fs6} can be transformed to those of \code{fs1} as shown.
...
\pkg{flexsurv} using the function \code{flexsurvspline}, and is also available in the Stata package \pkg{stpm2} \citep{stpm2} (historically \pkg{stpm}, \citet{stpm:orig,stpm:update}).
...
\code{dsurvspline} and \code{psurvspline}.
...
<<splinehaz,include=FALSE,fig=TRUE>>=
...
\begin{figure}[h]
  \centering
  \includegraphics{flexsurv-splinehaz}
  \caption{Comparison of spline and generalized gamma fitted hazards.}
  \label{fig:spline:haz}
\end{figure}
Note that the log hazard ratios, and their standard errors, under the proportional hazards spline
model with one internal knot, are practically the same as under a standard Cox model.
<<>>=
...
@ 
To complete the model, the internal knot is placed at the median uncensored log survival time,
and boundary knots are placed at the minimum and maximum. 
...
Given that an individual is in state $S(t)$ at time $t$, their next
state, and the time of the change, are governed by a set of
\emph{transition intensities}
\[q_{rs}(t,\mathbf{z}(t),\mathcal{F}_t) = \lim_{\delta t \rightarrow 0} P(S(t+\delta t) = s | S(t) = r, t, \mathbf{z}(t), \mathcal{F}_t) / \delta t \] 
...
series of event times $t_{1},\dots, t_{n}$ for an individual.  The
...
can be fitted with the \pkg{msm} package for R \citep{msmjss}, but are
...
does not depend further on $\mathcal{F}_t$.  Again any survival
...
\citet{putter:mstate} explain how to implement multi-state models by
...
Three multi-state models are fitted to the BOS data using \code{flexsurvreg}.  The
...
Any parametric distribution can be fitted, just as for standard
...
model.  A restriction is that all transition-specific models must be
...
\Sexpr{round(crexp$AIC)} for the exponential model.  For th e$2\rightarrow 3$ transition,
the clock-forward and clock-reset models give slightly
different hazard trajectories.
<<cumhaz,include=FALSE,fig=TRUE>>=
...
@ 
\begin{figure}[h]
\includegraphics{flexsurv-cumhaz}
...
$P() = I$ \citep{cox:miller}.  This can be solved numerically, as in
...
For Markov models, an approximate solution to the Kolmogorov equation
...
provide Stata software based on Gauss-Hermite
quadrature. Alternatively, a probabilistic modelling language such as
Stan \citep{stan-manual:2014} or BUGS \citep{bugs:book} would be
...
The examples in this paper were run using version 0.5 of \pkg{flexsurv}, available from \url{http://CRAN.R-project.org/package=flexsurv}.  Development versions are available on \url{https://github.com/chjackson/flexsurv-dev}, and contributions are welcome.
...
the package, and all users who have reported bugs and given suggestions.
